...
# Free OpenRouter models can be temporarily saturated by provider-side limits.
# Keep this list conservative and text-focused for resume/JD analysis.
OPENROUTER_RATE_LIMIT_FALLBACK_MODELS: tuple[str, ...] = (
...
def _is_rate_limit_error(exc: Exception) -> bool:
    """Return True when an upstream exception is a provider rate limit."""
    ...
    return any(
        marker in message
        for marker in ("429", "ratelimit", "rate limit", "rate-limited")
    )


def _openrouter_model_sequence(config: LLMConfig) -> list[str]:
    """Build the ordered OpenRouter model list, starting with the configured one."""
    models = [config.model, *OPENROUTER_RATE_LIMIT_FALLBACK_MODELS]
    seen: set[str] = set()
    sequence: list[str] = []
...
def _config_with_model(config: LLMConfig, model: str) -> LLMConfig:
    """Clone an LLM config with a different model."""
    return LLMConfig(
...
async def _acompletion_with_rate_limit_fallback(
    config: LLMConfig,
    kwargs: dict[str, Any],
    *,
    router: Router | None = None,
) -> Any:
    """Run a completion and fallback across OpenRouter free models on 429.

    Router retries still handle transient failures for the configured model.
    This helper only moves to the next model after a final rate-limit error.
    """
    if config.provider != "openrouter":
        if router:
    ...
    for index, model in enumerate(_openrouter_model_sequence(config)):
        try:
            if index == 0 and router:
                return await router.acompletion(**kwargs)

            attempt_config = _config_with_model(config, model)
            attempt_kwargs = dict(kwargs)
            attempt_kwargs["model"] = get_model_name(attempt_config)
            attempt_kwargs["api_key"] = _effective_api_key(
                attempt_config.provider, attempt_config.api_key
            )
            attempt_kwargs["api_base"] = _normalize_api_base(
                attempt_config.provider, attempt_config.api_base
            )

            if model != config.model:
                logging.warning(
                    "OpenRouter primary model rate-limited; trying fallback model %s",
                    model,
                )
            ...
            if not _is_rate_limit_error(exc):
                raise
            last_rate_limit = exc
            logging.warning("OpenRouter model %s rate-limited", model)

    if last_rate_limit:
        raise last_rate_limit
    raise RuntimeError("OpenRouter fallback sequence is empty")
...
async def check_llm_health(
    ...
    *,
    ...
    try:
        ...
        response = await _acompletion_with_rate_limit_fallback(config, kwargs)
...
async def complete(
    ...
    try:
        ...
        response = await _acompletion_with_rate_limit_fallback(
            config, kwargs, router=router
        )
